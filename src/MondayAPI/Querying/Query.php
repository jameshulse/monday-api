...
class Query
{
    public static function validate($field, $field_typedata)
    {
        if ($field_typedata['type'] === 'Int' || $field_typedata['type'] === '!Int' || $field_typedata['type'] === '[Int]') {
            return true;
        } elseif ($field_typedata['type'] === 'String' || $field_typedata['type'] === '!String') {
            return true;
        } elseif ($field_typedata['type'] === 'Boolean') {
            return true;
        } elseif (isset($field_typedata['object'])) {
        ...
        } elseif ($field_typedata['type'][0] === '[' && $field_typedata['type'][strlen($field_typedata['type']) - 1] === ']') {
        } else {
            return true;
        }
